...
CREATE TABLE Licencias(licNumero int identity not null,
 ...
 constraint fk_EstLic foreign key(estNumero) references
Establecimientos(estNumero),
...
CREATE TABLE Inspecciones(inspID int identity not null,
 ...
 constraint fk_EstabInsp foreign key(estNumero)
references Establecimientos(estNumero),
 constraint fk_ViolInsp foreign key(violCodigo)
references TipoViolacion(violCodigo),
 constraint ck_Riesgo check(inspRiesgo
IN('Bajo','Medio','Alto')),
 constraint ck_Result check(inspResultado IN('Pasa',
'Falla', 'Pasa con condiciones', 'Oficina no encontrada')))
GO